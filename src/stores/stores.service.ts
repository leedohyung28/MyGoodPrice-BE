...
export class StoresService {
  ...
  async getStoresBy(
    queryParams: GetStoresQueryDTO,
  ): Promise<StoresReturnDTO[]> {
    const { category, location, search } = queryParams;
    ...
    const results = await this.storesRepository.findOptions(query, {
      limit,
      skip: page_new * limit_new,
    });
